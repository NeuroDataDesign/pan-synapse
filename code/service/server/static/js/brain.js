...
var brain = document.querySelector('path.brain'), length, timer;
var sil = document.querySelector('path.sil'), length, timer;

var obj = {length:0,
           pathLength:brain.getTotalLength()};


setTimeout(function() {
brain.style.stroke = '#111111';
var t = TweenMax.to(obj, 22, {length:obj.pathLength, onUpdate:drawLine, ease:Linear.easeNone})
},1000)

function drawLine() {
  brain.style.strokeDasharray = [obj.length,obj.pathLength].join(' ');
  setTimeout(function() {
  brain.style.fill = '#111111';
  },6500);
  setTimeout(function() {
  sil.style.fill = '#111111';
  },9500);
  setTimeout(function() {
    jQuery("svg").attr("class", "scale");
brain.style.stroke = 'transparent';
  },11500);
  setTimeout(function() {
    jQuery("body").attr("class", "aftersil");
  },12500);
}
...
function init() {
    ...
    var r = $i("starfield");
    r.style.position = "absolute", r.width = w, r.height = h, context = r.getContext("2d"), context.fillStyle = "#111111", context.strokeStyle = "rgb(255,255,255)"
}
function anim() {
